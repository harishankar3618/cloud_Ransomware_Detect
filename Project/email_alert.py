...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("email_alert.log"),
...
def load_config():
    ...
    config_path = '/var/www/cloud_Ransomware_Detect/Project/config.json'
    try:
...
def send_mail(receipt_email, result):
    ...
    try:
        ...
        if not sender_email or not sender_password:
            raise ValueError("Email credentials not found in config")
        ...
        logging.info(f"Email sent successfully to: {receipt_email}")
        print("Email sent successfully. Check your inbox for results.")

    except Exception as e:
        logging.error(f"Failed to send malware alert email: {e}")
        print(f"Failed to send email: {e}")
        sys.exit(1)

def read_results_from_file(file_path):
    """Read scan results from temporary file"""
    try:
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python email_alert.py <results_file_path> <receipt_email>")
        sys.exit(1)
    ...
    if '@' not in receipt_email or '.' not in receipt_email:
        print("Invalid email address format")
        sys.exit(1)
    
    try:
        result = read_results_from_file(results_file_path)
